import React, {useState} from 'react';

//Шаг
export default function Counter({width, minValue, maxValue, step, useInput, disabled}) {
  const [focus, setFocus] = useState(false);

  const increase = () => {
    if (useInput.value === '') {
      useInput.onChange(minValue);
    } else if (useInput.value === maxValue) {
      useInput.onChange(minValue);
    } else if (useInput.value < maxValue) {
      useInput.onChange(useInput.value + 1);
    }
  };

  const decrease = () => {
    if (useInput.value === '') {
      useInput.onChange(maxValue);
    } else if (useInput.value === minValue) {
      useInput.onChange(maxValue);
    } else if (useInput.value > minValue) {
      useInput.onChange(useInput.value - 1);
    }
  };

  const innerOnBlur = () => {
    setFocus(false);
    useInput.onBlur();
  };

  const handleChange = event => {
    let value = event.target.value;
    if (value === '') {
      useInput.onChange('');
      return;
    }
    value = Number(value);
    if (!isNaN(value) && value >= minValue && value <= maxValue) {
      useInput.onChange(value);
    }
  };

  const className = () => {
    let className = 'custom-input flex-row align-items-center';
    if (focus) {
      className += ' custom-input-active';
    }
    if (!useInput.validated.all) {
      className += ' custom-input-warning';
    }
    return className;
  };

  return (
    <div className={className()} style={{width}}>
      <input
        type="text"
        className="custom-inner-input"
        value={useInput.value}
        disabled={disabled}
        onFocus={() => setFocus(true)}
        onBlur={innerOnBlur}
        onChange={handleChange}
      />
      <div className="flex-column center">
        <button
          className="custom-counter-icon-btn"
          onClick={increase}
          onFocus={() => setFocus(true)}
          onBlur={innerOnBlur}
          disabled={disabled}
        >
          <svg
            style={{position: 'relative', top: '-2px'}}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 320 512"
            height="12"
            width="10"
          >
            <path d="M182.6 41.4c-12.5-12.5-32.8-12.5-45.3 0l-128 128c-9.2 9.2-11.9 22.9-6.9 34.9s16.6 19.8 29.6 19.8H288c12.9 0 24.6-7.8 29.6-19.8s2.2-25.7-6.9-34.9l-128-128z" />
          </svg>
        </button>
        <button
          className="custom-counter-icon-btn"
          onClick={decrease}
          onFocus={() => setFocus(true)}
          onBlur={innerOnBlur}
          disabled={disabled}
        >
          <svg
            style={{position: 'relative', top: '-4px'}}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 320 512"
            height="12"
            width="10"
          >
            <path d="M182.6 470.6c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-9.2-9.2-11.9-22.9-6.9-34.9s16.6-19.8 29.6-19.8H288c12.9 0 24.6 7.8 29.6 19.8s2.2 25.7-6.9 34.9l-128 128z" />
          </svg>
        </button>
      </div>
    </div>
  );
}
